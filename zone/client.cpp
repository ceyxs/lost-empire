...
void Client::IncreaseLanguageSkill(int skill_id, int value) {
	...
	if (m_pp.languages[skill_id] > 200) //Lang skill above max
		m_pp.languages[skill_id] = 200;
...
void Client::CheckLanguageSkillIncrease(uint8 langid, uint8 TeacherSkill) {
	//using languages for gathering skills
	return;
	if (IsDead() || IsUnconscious())
		return;
	if (IsAIControlled())
...
int Client::GetItemMeleeDmg() {
	int amt = itembonuses.ProcChance;
	return amt;
}

int Client::GetItemSpellDmg() {
	int amt = itembonuses.SpellDmg;
	return amt;
}
