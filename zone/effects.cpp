...
int32 Mob::GetActDoTDamage(uint16 spell_id, int32 value, Mob* target) {
	...
	if (IsClient()) { //Charisma to crit for dots
		int chabonus;
		if (spells[spell_id].skill == EQ::skills::SkillTigerClaw) {
			chabonus = (CastToClient()->GetDEX() - 75); // dex based for rogue dots
		}
		else {
	...
	if (chance > 0 && (zone->random.Roll(chance))) {
	 ...
	 if (IsClient()) {
		...
		if (intbon > 0) {
			...
			value += extra;
			if (CastToClient()->Admin() > 1) {
				Message(Chat::FocusEffect, "Scale Stat %i. %i base dot damage, %i bonus damage", (intbon + 75), -base, -extra);
			}
		}
	}
...
bool Client::UseDiscipline(uint32 spell_id, uint32 target) {
	...
	int adj_end_cost = (GetMaxEndurance() * spells[spell_id].EndurCost * .01);
	if (adj_end_cost > GetEndurance()) {
