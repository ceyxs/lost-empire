...
)
{
	switch (event) {
			...
			if (extra_pointers) {
				...
				for (size_t i  = 0; i < sz; ++i) {
					...
					if (inst) {
						...
						temp_var_name = var_name;
						temp_var_name += "_augmented";
						ExportVar(package_name.c_str(), temp_var_name.c_str(), inst->IsAugmented());
					}
					else {
		...
		case EVENT_MELEE_CRIT: {
			ExportVar(package_name.c_str(), "target_id", data);
			break;
		}
