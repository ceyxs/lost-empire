...
XS(XS_QuestItem_IsAugmentable); /* prototype to pass -Wmissing-prototypes */
XS(XS_QuestItem_IsAugmentable) {
	dXSARGS;
	if (items != 1)
		Perl_croak(aTHX_ "Usage: QuestItem::IsAugmentable(THIS)");
	{
		EQ::ItemInstance *THIS;
		uint32 RETVAL;
		dXSTARG;
...
XS(boot_QuestItem) {
	...
	newXSproto(strcpy(buf, "IsAugmentable"), XS_QuestItem_IsAugmentable, file, "$");
