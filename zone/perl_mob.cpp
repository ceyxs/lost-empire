...
XS(XS_Mob_AddBuff); /* prototype to pass -Wmissing-prototypes */
XS(XS_Mob_AddBuff) {
	dXSARGS;
	if (items < 4 || items > 5)
		Perl_croak(aTHX_ "Usage: Mob::AddBuff(THIS, Mob* caster, int spellid, int duration, [int level_override = -1 ])");
	{
		Mob *THIS;
		Mob *caster;
		int spellid = (uint32)SvUV(ST(2));
		int duration = (uint32)SvUV(ST(3));;
		int level_override;
		...
		if (items < 5)
			...
			level_override = (int32)SvIV(ST(4));
		}

		THIS->AddBuff(caster, spellid, duration, level_override);
	}
	XSRETURN_EMPTY;
}
...
XS(boot_Mob) {
	...
	newXSproto(strcpy(buf, "AddBuff"), XS_Mob_AddBuff, file, "$$$$;$");
	XSRETURN_YES;
}
