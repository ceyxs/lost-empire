						...
						uint32 inventory_slot, int16 resist_adjust, bool isproc, int level_override)
{
	...
	if (spells[spell_id].EndurCost && !isproc) {
		...
		if (IsClient() && CastToClient()->Admin() > 1) {
			Message(Chat::FocusEffect, "Base end cost: %i", end_cost);
		}
		int adj_end_cost = (GetMaxEndurance() * spells[spell_id].EndurCost * .01);
		if (IsClient() && CastToClient()->Admin() > 1) {
			Message(Chat::FocusEffect, "max endurance: %i / adjusted end cost: %i", GetMaxEndurance(), adj_end_cost);
		}
		SetEndurance(GetEndurance() - EQ::ClampUpper(adj_end_cost, GetEndurance()));
