...
void ZoneDatabase::DeleteDoor(uint32 did) {
	if (did == 0) {
		return;
	}

	std::string query = StringFormat("DELETE FROM doors WHERE id = %i", did);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		LogError("Unable to delete door: [{}]", results.ErrorMessage().c_str());
	}
}
