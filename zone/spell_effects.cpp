		...
		switch(effect)
		{
			...
			case SE_SkillAttack:
			{
				...
				*/
				...
				/*Commenting out for now, handling these abilities with lua scripts
				// bow special
				if (caster->IsClient() && spells[spell_id].base[i] > 299) {
					EQ::ItemInstance* bow = nullptr;
					bow = caster->CastToClient()->GetInv().GetItem(EQ::invslot::slotRange);
					int bowdamage = 0;
					int leveldamage = (caster->GetLevel() * spells[spell_id].max[i]) / 100;
					if (bow != nullptr && bow->GetItem()->ItemType == EQ::item::ItemTypeBow) {
						bowdamage = bow->GetItem()->Damage * ((spells[spell_id].base[i] - 200) / 100);
						int totaldmg = leveldamage + bowdamage;
						if (caster->CastToClient()->Admin() > 1) {
							caster->Message(Chat::FocusEffect, "Bow Damage %i level damage %i total damage %i", bowdamage, leveldamage, totaldmg);
						}
						caster->DoMeleeSkillAttackDmg(this, totaldmg, EQ::skills::SkillArchery, 10000, focus, false);
						break;
					}
					else {
						caster->Message(13, "You cannot perform this action with no bow.");
						break;
					}
				}
				// greater than 200, bash special.
				else if (caster->IsClient() && spells[spell_id].base[i] > 199) {
					EQ::ItemInstance* shield = nullptr;
					shield = caster->CastToClient()->GetInv().GetItem(EQ::invslot::slotSecondary);
					int bashdamage = 0;
					int leveldamage = (caster->GetLevel() * spells[spell_id].max[i]) / 100;
					if (shield != nullptr && shield->GetItem()->ItemType == EQ::item::ItemTypeShield) {
						bashdamage = (shield->GetItem()->AC * (spells[spell_id].base[i] - 100)) / 100;
						int totaldmg = leveldamage + bashdamage;
						if (caster->CastToClient()->Admin() > 1) {
							caster->Message(Chat::FocusEffect, "Shield Damage %i level damage %i total damage %i", bashdamage, leveldamage, totaldmg);
						}
						caster->DoMeleeSkillAttackDmg(this, totaldmg, EQ::skills::SkillBash, 10000, focus, false);
						break;
					}
					else {
						caster->Message(13, "You cannot perform this action with no shield.");
						break;
					}
				}
				//special code for > 99, scale skill attack based on weapon dmg + 1/2 level.
				else if (caster->IsClient() && spells[spell_id].base[i] > 99) {
					EQ::ItemInstance* weapon = nullptr;
					weapon = caster->CastToClient()->GetInv().GetItem(EQ::invslot::slotPrimary);
					int weapondamage = 0;
					EQ::skills::SkillType weaponskill = EQ::skills::SkillHandtoHand;
					int leveldamage = (caster->GetLevel() * spells[spell_id].max[i]) * .01;
					if (weapon != nullptr && weapon->IsWeapon()) {
						weapondamage = weapon->GetItem()->Damage * ((spells[spell_id].base[i]) / 100);
						int totaldmg = leveldamage + weapondamage;
						if (caster->CastToClient()->Admin() > 1) {
							caster->Message(Chat::FocusEffect, "Weapon Damage %i level damage %i total damage %i", weapondamage, leveldamage, totaldmg);
						}
						switch (weapon->GetItem()->ItemType) {
						case EQ::item::ItemType1HSlash: // 1H Slashing
							weaponskill = EQ::skills::Skill1HSlashing;
							break;
						case EQ::item::ItemType2HSlash: // 2H Slashing
							weaponskill = EQ::skills::Skill2HSlashing;
							break;
						case EQ::item::ItemType1HPiercing: // Piercing
							weaponskill = EQ::skills::Skill1HPiercing;
							break;
						case EQ::item::ItemType1HBlunt: // 1H Blunt
							weaponskill = EQ::skills::Skill1HBlunt;
							break;
						case EQ::item::ItemType2HBlunt: // 2H Blunt
							weaponskill = EQ::skills::Skill2HBlunt;
							break;
						case EQ::item::ItemType2HPiercing: // 2H Piercing
							weaponskill = EQ::skills::Skill2HPiercing;
							break;
						case EQ::item::ItemTypeMartial:
							weaponskill = EQ::skills::SkillHandtoHand;
							break;
						default:
							weaponskill = EQ::skills::SkillHandtoHand;
							break;
						}
						caster->DoMeleeSkillAttackDmg(this, totaldmg, weaponskill, 10000, focus, false);
						break;
					}
					else {
						caster->Message(13, "You cannot perform this action with no weapon.");
						break;
					}
				}
				*/
...
#endif
				if(IsClient()) {
					int value = (GetMaxEndurance() * effect_value) / 100;
					CastToClient()->SetEndurance(CastToClient()->GetEndurance() + value);
					if (effect_value < 0)
			      ...
			      int ticsremaining, uint16 caster_id)
{
	...
	effect_value = CalcSpellEffectValue_formula(formula, base, max, caster_level, spell_id, caster, ticsremaining);
...
int Mob::CalcSpellEffectValue_formula(int formula, int base, int max, int caster_level, uint16 spell_id, Mob* caster, int ticsremaining)
{
/*
...
*/
	...
	switch(formula)
	{
		...
		case 111:
			if (caster && caster->IsClient()) {
				if (spells[spell_id].classes[caster->GetClass() - 1] > 50) {
					result = ubase;
				}
				else {
					result = updownsign * (ubase + (caster_level - spells[spell_id].classes[caster->GetClass() - 1]));
				}
			}
			else {
				result = ubase;
